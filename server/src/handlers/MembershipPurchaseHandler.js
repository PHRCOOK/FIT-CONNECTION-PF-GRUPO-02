const { createMembershipPurchase, getMembershipPurchases, getMembershipPurchaseById, getMembershipPurchasesByUserId, updateMembershipPurchase, deleteMembershipPurchase } = require('../controllers/MembershipPurchaseController')

const createMembershipPurchaseHandler = async (req, res) => {
    const { membership_id, user_id, payment_method } = req.body
    
    try {
        const purchase = await createMembershipPurchase(Number(user_id), membership_id, payment_method)
